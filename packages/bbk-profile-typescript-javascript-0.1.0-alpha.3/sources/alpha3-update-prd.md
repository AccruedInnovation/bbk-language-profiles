# TypeScript/JavaScript profile alpha.3 update PRD

**Document status:** implementation-ready update requirements  
**Target package:** `bbk-profile-typescript-javascript 0.1.0-alpha.3`  
...
| Field | Value |
|---|---|
| Source package | `bbk-profile-typescript-javascript 0.1.0-alpha.2` |
| Source archive SHA-256 | `b5391b4d4c1c96f5e8acdd12b9063b34d2e1efc105dd788c706681b7daa29c5a` |
| Source package root SHA-256 | `e56a1d0e90bf1848b02332e381e2416067b998226cc53f7e98758be26a63fdf5` |
| Source manifested files | 146 |
| Existing skill count | 13 |
...
```json
{
  ...
  "capabilities": {
    "state_decision_effect": {
      ...
      "representations": [
        "discriminated unions and products of independent unions",
        "branded identities and boundary types",
        "runtime schema/decoder boundaries distinct from static types",
        "reducers, application services, or explicit state-machine boundaries"
      ],
      "formal_model_tools": [
        "property/model-based tests supported by the repository",
        "fake-clock and scheduler-controlled async traces",
        "browser/server harnesses and protocol trace replay",
        "optional statechart or external Quint/TLA+ mappings"
      ],
      "limitations": [
...
The profile must explain and project these TypeScript/JavaScript concepts:

- discriminated unions and products of independent unions
- branded identities and boundary types
- runtime schema/decoder boundaries distinct from static types
- reducers, application services, or explicit state-machine boundaries
- AbortSignal, task, subscription, timer, worker, stream, and cleanup ownership
- browser, server, CLI, package, plugin, and OMP-extension effect adapters
...
|---|---|
| `architecture-boundary` | comprehensive-analysis-tsjs, only for an explicit broad survey |
| `interface-consumer-compatibility` | tsjs-api-module-package-review plus tsjs-type-contract-review and/or tsjs-runtime-data-contract-review as applicable |
...
| `operations-performance-resource` | tsjs-operational-release-review; add tsjs-browser-ui-review for browser/UI subjects |
| `package-install-migration-release` | tsjs-api-module-package-review and tsjs-operational-release-review |
...
- workspace/package manifests, lockfile and repository-selected runtime/package manager
- TypeScript/JavaScript config inheritance and strictness policy
- exports, declaration output, runtime schemas, built artifacts and package contents
- browser/server/worker/extension boundaries
- async resources, cancellation, streams, subscriptions and cleanup
- consumer fixtures, test configuration and repository commands
...
- typecheck, transform/build, declaration emit, test and package-consumer evidence kept distinct
- runtime schema/decoder and actual artifact execution evidence
- browser, worker, stream, abort, cleanup and package-install checks when applicable
- tsjs-evidence-reproducer as the principal EvidenceReceipt v2 adapter
...
```text
immutable bbk-profile-typescript-javascript-0.1.0-alpha.3.zip
SHA-256 companion
package manifest
release notes
migration guide
