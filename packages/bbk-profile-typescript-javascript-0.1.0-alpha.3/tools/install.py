#!/usr/bin/env python3
"""Cautious installer for bbk-profile-typescript-javascript across Codex, OMP, and Claude Code."""
...
def atomic_write(path: Path, data: bytes, mode: int | None = None) -> None:
    ...
    temp = path.with_name(f".{path.name}.bbk-tsjs-install-{os.getpid()}")
...
def backup_path(backup_root: Path, destination: Path) -> Path:
    anchor = destination.anchor.replace(":", "")
    return backup_root / anchor / str(destination).lstrip("/\\")
...
def install_file(source: Path, destination: Path, **kwargs: Any) -> None:
    install_bytes(source.read_bytes(), destination, source=str(source), executable=source.suffix == ".py" or bool(source.stat().st_mode & 0o111), **kwargs)
...
def install(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.scope == "user":
        ...
        omp_extension = Path.home() / ".omp" / "agent" / "extensions" / "bbk-profile-typescript-javascript"
        binaries = bin_dir()
    else:
        assert project is not None
        agent_skills = project / ".agents" / "skills"
        claude_skills = project / ".claude" / "skills"
        omp_extension = project / ".omp" / "extensions" / "bbk-profile-typescript-javascript"
        binaries = None
    ...
    if binaries is not None:
        ...
        install_bytes(content, binaries / name, source="generated:bbk-tsjs-launcher", executable=True, **common)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print(f"bbk-profile-typescript-javascript install: error: {exc}", file=sys.stderr)
        return 2
