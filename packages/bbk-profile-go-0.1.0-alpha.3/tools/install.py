#!/usr/bin/env python3
"""Cautious installer for bbk-profile-go across Codex, OMP, and Claude Code."""
...
def atomic_write(path: Path, data: bytes, mode: int | None = None) -> None:
    ...
    temp = path.with_name(f".{path.name}.bbk-go-install-{os.getpid()}")
...
def install(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.scope == "user":
        ...
        omp_extension = Path.home() / ".omp" / "agent" / "extensions" / "bbk-profile-go"
        binaries = bin_dir()
    else:
        assert project is not None
        agent_skills = project / ".agents" / "skills"
        claude_skills = project / ".claude" / "skills"
        omp_extension = project / ".omp" / "extensions" / "bbk-profile-go"
        binaries = None
    ...
    if binaries is not None:
        ...
        install_bytes(content, binaries / name, source="generated:bbk-go-launcher", executable=True, **common)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print(f"bbk-profile-go install: error: {exc}", file=sys.stderr)
        return 2
