# Rust profile alpha.3 update PRD

**Document status:** implementation-ready update requirements  
**Target package:** `bbk-profile-rust 0.1.0-alpha.3`  
...
| Field | Value |
|---|---|
| Source package | `bbk-profile-rust 0.1.0-alpha.2` |
| Source archive SHA-256 | `e2d7dbf0ff9d3d3a5827e5577eb52f52eebd5c763c1b5b14c1158b3388b948e1` |
| Source package root SHA-256 | `1dc73a7d1aa3a064b14dbd43b8e899779e0e178cf97ba7f9f23a404b3d5c18ea` |
| Source manifested files | 357 |
...
```json
{
  ...
  "capabilities": {
    "state_decision_effect": {
      ...
      "representations": [
        "enum and product-of-enum state",
        "newtype identities and units",
        "explicit error/result enums",
        "application-service decision functions"
      ],
      "formal_model_tools": [
        "property/state-machine tests such as proptest or repository equivalents",
        "Loom-like deterministic concurrency exploration where applicable",
        "Miri for supported unsafe/aliasing claims",
        "Kani or other qualified bounded verification where selected",
        "optional external Quint/TLA+ trace mappings"
      ],
      "limitations": [
...
The profile must explain and project these Rust concepts:

- enum and product-of-enum state
- newtype identities and units
- explicit error/result enums
- application-service decision functions
- ports/adapters and trait boundaries
- task, channel, lease, cancellation, shutdown, and persistence ownership
...
|---|---|
| `architecture-boundary` | comprehensive-analysis-rust, only for explicit broad architecture surveys |
...
| `security-privacy-supply-chain` | rust-security-supply-chain-review; add rust-unsafe-ffi-assurance when unsafe/FFI/ABI is in scope |
...
- Cargo workspace metadata and manifests
- toolchain and MSRV pins
- changed crates/modules and reverse consumers
- public exports, feature/cfg surfaces, serialization and migration contracts
- unsafe, FFI, ABI, build.rs and proc-macro surfaces
- task/channel/cancellation/persistence owners
- focused tests, fixtures, lockfile and repository-defined commands
...
- repository-authoritative fmt/check/lint/test commands
- consumer/API/feature-matrix/package checks
- Miri, Loom, fuzz, audit, semver or benchmark evidence only when applicable and available
- rust-evidence-reproducer as the principal EvidenceReceipt v2 adapter
...
```text
immutable bbk-profile-rust-0.1.0-alpha.3.zip
SHA-256 companion
package manifest
release notes
migration guide
