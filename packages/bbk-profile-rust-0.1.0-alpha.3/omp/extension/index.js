...
function readCurrent(currentPath) {
  try {
    ...
    if (value?.id === "rust" && typeof value.path === "string") return value.path;
  } catch {
...
function profileRoot(cwd) {
  if (process.env.BBK_PROFILE_RUST_ROOT) return path.resolve(process.env.BBK_PROFILE_RUST_ROOT);
  for (const root of ancestors(cwd)) {
    for (const current of [
      path.join(root, ".bbk", "profiles", "rust", "current.json"),
      path.join(root, ".bbk-kit", "profiles", "rust", "current.json"),
    ]) {
...
function command(cwd) {
  ...
  return { executable: process.platform === "win32" ? "bbk-rust.cmd" : "bbk-rust", prefix: [] };
}

function runRust(args, cwd, signal) {
  return new Promise((resolve, reject) => {
    ...
    child.on("close", code => {
      ...
      try {
        ...
        details = { status: "ERROR", stdout, stderr, parseError: "bbk-rust did not return JSON" };
      }
...
function commonArgs(params) {
  return [
    ...
    ...(params.structureContracts || []).flatMap(value => ["--structure-contract", value]),
    ...(params.executionSlices || []).flatMap(value => ["--execution-slice", value]),
...
function registerTool(pi, definition) {
  pi.registerTool({
    ...
    async execute(_id, params, signal, _onUpdate, ctx) {
      return toolResult(await runRust(definition.argv(params), ctx.cwd, signal));
    },
  });
}
...
export default function bbkRustExtension(pi) {
  const { z } = pi.zod;
  pi.setLabel("BBK Rust Profile");
  const parameters = z.object({
    ...
    structureContracts: z.array(z.string()).optional(),
    executionSlices: z.array(z.string()).optional(),
    runTools: z.boolean().optional(),
  });

  pi.registerTool({
    name: "bbk_rust_state_effect",
    label: "BBK Rust State–Decision–Effect Projection",
  ...
  pi.registerTool({
    name: "bbk_rust_state_effect_inventory",
    label: "BBK Rust State–Decision–Effect Inventory",
  ...
  pi.registerTool({
    name: "bbk_rust_state_effect_review",
    label: "BBK Rust State–Decision–Effect Review",
  ...
  pi.registerTool({
    name: "bbk_rust_review_context",
    label: "BBK Rust Review Context",
  ...
  pi.registerTool({
    name: "bbk_rust_review_lens",
    label: "BBK Rust Review Lens",
  ...
  pi.registerTool({
    name: "bbk_rust_evidence_adapter",
    label: "BBK Rust Evidence Adapter",
  ...
  registerTool(pi, {
    name: "bbk_rust_preflight",
    label: "BBK Rust Preflight",
    description: "Inspect the effective Cargo workspace, toolchain declaration, supported configurations, and repository commands without granting or running project effects by default.",
  ...
  registerTool(pi, {
    name: "bbk_rust_resolve",
    label: "BBK Rust Resolve",
    description: "Resolve proportional Rust skills, focused review packs, and planned gates from the role, task, changed scope, and assurance tier.",
    parameters,
    argv: p => ["resolve", ...commonArgs(p)],
  });
  registerTool(pi, {
    name: "bbk_rust_gate_plan",
    label: "BBK Rust Gate Plan",
    description: "Compile a non-executing, repository-aware Rust gate plan. Tool installation and effects remain separately authorized.",
    parameters,
    argv: p => ["gate-plan", ...commonArgs(p)],
  });
  registerTool(pi, {
    name: "bbk_rust_structure",
    label: "BBK Rust Structure Projection",
    description: "Project an authoritative generic ImplementationStructureContract into Rust workspace, type, ownership, failure, and test-seam vocabulary without repository mutation.",
    parameters: z.object({ root: z.string().optional(), contract: z.string(), role: z.string().optional(), taskProfile: z.string().optional(), assuranceTier: z.enum(["routine", "material", "consequential", "critical"]).optional() }),
    argv: p => ["structure", ...(p.root ? ["--root", p.root] : []), "--contract", p.contract, ...(p.role ? ["--role", p.role] : []), ...(p.taskProfile ? ["--task-profile", p.taskProfile] : []), ...(p.assuranceTier ? ["--assurance-tier", p.assuranceTier] : [])],
  });
  registerTool(pi, {
    name: "bbk_rust_slice",
    label: "BBK Rust Execution Slice Projection",
    description: "Project an authoritative generic ExecutionSlice into Rust touchpoints, dependency closure, evidence, candidate boundaries, and scaffolding dispositions without executing gates.",
    parameters: z.object({ root: z.string().optional(), slice: z.string(), role: z.string().optional(), taskProfile: z.string().optional(), assuranceTier: z.enum(["routine", "material", "consequential", "critical"]).optional() }),
    argv: p => ["slice", ...(p.root ? ["--root", p.root] : []), "--slice", p.slice, ...(p.role ? ["--role", p.role] : []), ...(p.taskProfile ? ["--task-profile", p.taskProfile] : []), ...(p.assuranceTier ? ["--assurance-tier", p.assuranceTier] : [])],
  });
  registerTool(pi, {
    name: "bbk_rust_structure_review",
    label: "BBK Rust Structure Review",
    description: "Compare planned and actual consequential Rust structure while preserving explicitly delegated private freedom. This returns findings, not acceptance authority.",
    parameters: z.object({ root: z.string().optional(), contract: z.string(), candidate: z.string(), actualInventory: z.string().optional(), assuranceTier: z.enum(["routine", "material", "consequential", "critical"]).optional() }),
    argv: p => ["structure-review", ...(p.root ? ["--root", p.root] : []), "--contract", p.contract, "--candidate", p.candidate, ...(p.actualInventory ? ["--actual-inventory", p.actualInventory] : []), ...(p.assuranceTier ? ["--assurance-tier", p.assuranceTier] : [])],
  });

  pi.registerCommand("bbk:rust:state-effect", {
    description: "Run state-effect; argument is the exact BBK profile-capability request JSON path",
    handler: async (args, ctx) => {
      const request = args?.trim();
      if (!request) { ctx.ui.notify("Usage: /bbk:rust:state-effect <request.json>", "warning"); return undefined; }
  ...
  pi.registerCommand("bbk:rust:state-effect-inventory", {
    description: "Run state-effect-inventory; argument is the exact BBK profile-capability request JSON path",
    handler: async (args, ctx) => {
      const request = args?.trim();
      if (!request) { ctx.ui.notify("Usage: /bbk:rust:state-effect-inventory <request.json>", "warning"); return undefined; }
  ...
  pi.registerCommand("bbk:rust:state-effect-review", {
    description: "Run state-effect-review; argument is the exact BBK profile-capability request JSON path",
    handler: async (args, ctx) => {
      const request = args?.trim();
      if (!request) { ctx.ui.notify("Usage: /bbk:rust:state-effect-review <request.json>", "warning"); return undefined; }
  ...
  pi.registerCommand("bbk:rust:review-context", {
    description: "Run review-context; argument is the exact BBK profile-capability request JSON path",
    handler: async (args, ctx) => {
      const request = args?.trim();
      if (!request) { ctx.ui.notify("Usage: /bbk:rust:review-context <request.json>", "warning"); return undefined; }
  ...
  pi.registerCommand("bbk:rust:review-lens", {
    description: "Run review-lens; argument is the exact BBK profile-capability request JSON path",
    handler: async (args, ctx) => {
      const request = args?.trim();
      if (!request) { ctx.ui.notify("Usage: /bbk:rust:review-lens <request.json>", "warning"); return undefined; }
  ...
  pi.registerCommand("bbk:rust:evidence-adapter", {
    description: "Run evidence-adapter; argument is the exact BBK profile-capability request JSON path",
    handler: async (args, ctx) => {
      const request = args?.trim();
      if (!request) { ctx.ui.notify("Usage: /bbk:rust:evidence-adapter <request.json>", "warning"); return undefined; }
  ...
  pi.registerCommand("bbk:rust", {
    description: "Resolve the Rust profile for the current changed scope",
    handler: async (args, ctx) => {
      const hints = args?.trim() ? args.trim().split(/\s+/) : [];
      const value = await runRust(["resolve", ...hints.flatMap(hint => ["--hint", hint])], ctx.cwd);
      ctx.ui.notify(value.code === 0 ? "Rust profile resolved" : "Rust profile resolution failed", value.code === 0 ? "info" : "error");
      return undefined;
    },
  });
  pi.registerCommand("bbk:rust:preflight", {
    description: "Inspect the current Rust workspace without executing Cargo/rustup unless 'run-tools' is supplied",
    handler: async (args, ctx) => {
      const runTools = args?.trim() === "run-tools";
      const value = await runRust(["preflight", ...(runTools ? ["--run-tools"] : [])], ctx.cwd);
      ctx.ui.notify(value.code === 0 ? "Rust preflight completed" : "Rust preflight failed", value.code === 0 ? "info" : "error");
      return undefined;
    },
  });
  pi.registerCommand("bbk:rust:gates", {
    description: "Compile a Rust gate plan; optional argument is the assurance tier",
    handler: async (args, ctx) => {
      const tier = args?.trim();
      const value = await runRust(["gate-plan", ...(tier ? ["--assurance-tier", tier] : [])], ctx.cwd);
      ctx.ui.notify(value.code === 0 ? "Rust gate plan compiled" : "Rust gate planning failed", value.code === 0 ? "info" : "error");
      return undefined;
    },
  });
  pi.registerCommand("bbk:rust:structure", {
    description: "Project a generic implementation-structure contract: <contract.json>",
    handler: async (args, ctx) => {
      const contract = args?.trim() || "implementation-structure.json";
      const value = await runRust(["structure", "--contract", contract], ctx.cwd);
      ctx.ui.notify(value.code === 0 ? "Rust structure projected" : "Rust structure projection failed", value.code === 0 ? "info" : "error");
      return undefined;
    },
  });
  pi.registerCommand("bbk:rust:slice", {
    description: "Project a generic execution slice: <slice.json>",
    handler: async (args, ctx) => {
      const slice = args?.trim() || "execution-slice.json";
      const value = await runRust(["slice", "--slice", slice], ctx.cwd);
      ctx.ui.notify(value.code === 0 ? "Rust execution slice projected" : "Rust execution slice projection failed", value.code === 0 ? "info" : "error");
      return undefined;
    },
  });
  pi.registerCommand("bbk:rust:structure-review", {
    description: "Review planned versus actual structure: <contract.json> <candidate-manifest.json> [actual-inventory.json]",
    handler: async (args, ctx) => {
      const parts = (args?.trim() || "").split(/\s+/).filter(Boolean);
      const value = parts.length >= 2
        ? await runRust(["structure-review", "--contract", parts[0], "--candidate", parts[1], ...(parts[2] ? ["--actual-inventory", parts[2]] : [])], ctx.cwd)
        : { code: 2, details: { status: "ERROR", error: "expected contract and candidate paths" } };
      ctx.ui.notify(value.code === 0 ? "Rust structure review completed" : "Rust structure review failed", value.code === 0 ? "info" : "error");
      return undefined;
    },
  });
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    ctx.ui.notify(root ? `BBK Rust profile 0.1.0-alpha.3 loaded` : "BBK Rust profile extension loaded; profile package not found", root ? "info" : "warning");
  });
}
