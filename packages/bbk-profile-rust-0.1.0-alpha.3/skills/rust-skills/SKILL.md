...
Comprehensive guide for writing high-quality, idiomatic Rust code. Contains 265 rules across 26 categories. The bundled text is a versioned reference corpus, not a claim that every rule applies to every crate or compiler release. BBK resolves rules through `rules-index.json`, the repository's declared MSRV and toolchain, the changed scope, and the work-unit assurance contract.
...
## Rule Categories by Retrieval Priority

| Retrieval order | Category | Default lens | Prefix | Rules |
|----------|----------|--------|--------|-------|
| 1 | Ownership & Borrowing | Foundational | `own-` | 12 |
| 2 | Error Handling | Foundational | `err-` | 12 |
| 3 | Memory Optimization | Conditional, evidence-driven | `mem-` | 17 |
| 4 | Unsafe Code | Triggered soundness assurance | `unsafe-` | 7 |
| 5 | API Design | Public-boundary dependent | `api-` | 17 |
| 6 | Async/Await | Async-code dependent | `async-` | 18 |
| 7 | Concurrency | Concurrency dependent | `conc-` | 4 |
| 8 | Compiler Optimization | Conditional, benchmarked | `opt-` | 12 |
| 9 | Numeric & Arithmetic Safety | Domain and range dependent | `num-` | 5 |
| 10 | Type Safety | Design dependent | `type-` | 13 |
| 11 | Trait & Generics Design | Design dependent | `trait-` | 6 |
| 12 | Conversions | Boundary dependent | `conv-` | 3 |
| 13 | Const & Compile-Time | API/toolchain dependent | `const-` | 4 |
| 14 | Serde | Serialization dependent | `serde-` | 8 |
| 15 | Pattern Matching | Context dependent | `pat-` | 5 |
| 16 | Macros | Macro dependent | `macro-` | 8 |
| 17 | Closures | Context dependent | `closure-` | 5 |
| 18 | Collections | Workload dependent | `coll-` | 4 |
| 19 | Naming Conventions | Project convention | `name-` | 16 |
| 20 | Testing | Risk-driven assurance | `test-` | 15 |
| 21 | Documentation | Audience/API dependent | `doc-` | 12 |
| 22 | Observability | Operationally dependent | `obs-` | 7 |
| 23 | Performance Patterns | Conditional, benchmarked | `perf-` | 13 |
| 24 | Project Structure | Project convention | `proj-` | 14 |
| 25 | Clippy & Linting | Repository policy | `lint-` | 13 |
| 26 | Anti-patterns | Diagnostic reference | `anti-` | 15 |

---

## Quick Reference

### 1. Ownership & Borrowing (FOUNDATIONAL)
...
### 3. Memory Optimization (FOUNDATIONAL)
...
### 6. Async/Await (COMMON/TRIGGERED)
...
### 7. Concurrency (COMMON/TRIGGERED)
...
### 8. Compiler Optimization (COMMON/TRIGGERED)
...
- [`opt-codegen-units`](rules/opt-codegen-units.md) - Measure codegen-unit count as a build-time/runtime trade-off
...
### 9. Numeric & Arithmetic Safety (COMMON/TRIGGERED)
...
### 11. Trait & Generics Design (CONDITIONAL)
...
### 13. Const & Compile-Time (CONDITIONAL)
...
### 15. Pattern Matching (CONDITIONAL)
...
### 19. Naming Conventions (CONDITIONAL)
...
### 23. Performance Patterns (CONDITIONAL)
...
### 24. Project Structure (PROJECT-SPECIFIC)
...
### 25. Clippy & Linting (PROJECT-SPECIFIC)
...
## Cargo profile policy

Do not apply one universal release profile. `opt-level`, LTO, codegen units, panic strategy, debug information, stripping, overflow checks, and target CPU settings are independent operational and performance decisions.

Use the repository's current profiles unless the work unit explicitly concerns performance, size, diagnostics, packaging, or crash behavior. For a proposed change:

1. state the shipped workload and operational goal;
2. compare the current profile with one bounded alternative;
3. preserve compiler, target, flags, workload, samples, variance, binary size, build time, and diagnostic effects;
4. adopt the change only when evidence supports the actual objective.

Keep a symbols-enabled profiling or diagnostic profile when production incident investigation requires it. `panic = "abort"`, stripping, native CPU targeting, LTO, and one codegen unit are never implied defaults.
...
1. **Run profile preflight** to bind edition, MSRV, toolchain, workspace, features, targets, and repository conventions.
2. **Check relevant categories and `rules-index.json`** based on task type and observed triggers.
3. **Apply only applicable rules**; treat optimization, tool, crate, and style recommendations as conditional.
4. **Filter by applicability first.** Finding severity comes from the demonstrated consequence in this codebase, not the category label.
