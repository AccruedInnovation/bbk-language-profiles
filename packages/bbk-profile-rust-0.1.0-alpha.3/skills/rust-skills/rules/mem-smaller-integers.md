...
Oversized fields can matter in large collections, caches, wire formats, and persistent layouts. But representation width is also a domain, compatibility, arithmetic, alignment, and conversion contract. Choose a narrower type only after proving the value range, units, precision/error budget, overflow behavior, serialization impact, and measured layout benefit.
...
// Precision is a domain contract; do not narrow it from range alone.
```
...
// Narrow a numeric representation only after defining range, units,
// serialization compatibility, acceptable error, and conversion behavior.
struct QuantizedDisplayPoint {
    lat_microdegrees: i32,
    lon_microdegrees: i32,
}
// This is one explicit fixed-point contract, not a universal geospatial choice.
```
...
// Narrower fields reduce the payload; alignment still determines final size
struct Compact {
