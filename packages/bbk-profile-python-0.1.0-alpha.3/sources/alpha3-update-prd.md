# Python profile alpha.3 update PRD

**Document status:** implementation-ready update requirements  
**Target package:** `bbk-profile-python 0.1.0-alpha.3`  
...
| Field | Value |
|---|---|
| Source package | `bbk-profile-python 0.1.0-alpha.2` |
| Source archive SHA-256 | `7326bd46eec47d5051ccc4b2ff47b26b2d9d3ec4a4bc4754f8b62933a79331d9` |
| Source package root SHA-256 | `d109386e9e1805a935f52023069a11e8fcead276445b9fda469997078f070718` |
| Source manifested files | 107 |
| Existing skill count | 15 |
...
```json
{
  ...
  "capabilities": {
    "state_decision_effect": {
      ...
      "representations": [
        "Enum and sum-like dataclass/validated-model state",
        "products of independent state dimensions",
        "immutable value objects where useful",
        "Protocol/ABC and package/API boundaries"
      ],
      "formal_model_tools": [
        "Hypothesis stateful or property testing when repository-qualified",
        "deterministic/fake-clock async and retry traces",
        "multiprocess/subprocess harnesses",
        "optional external state-machine or Quint/TLA+ mappings"
      ],
      "limitations": [
        "The profile must not create or replace the project environment or select an interpreter.",
        "Typing evidence is distinct from runtime validation and packaging evidence.",
        "Native extension, multi-platform, process, async, web-stack and deployment claims remain conditional on exact environment qualification."
      ]
    },
    "review_assurance": {
      "status": "supported",
...
The profile must explain and project these Python concepts:

- Enum and sum-like dataclass/validated-model state
- products of independent state dimensions
- immutable value objects where useful
- Protocol/ABC and package/API boundaries
- asyncio task, cancellation, queue, process, context-manager and resource ownership
- exception/result/disposition and persistence/migration boundaries
...
|---|---|
| `architecture-boundary` | comprehensive-analysis-python, only for explicit broad surveys |
| `interface-consumer-compatibility` | python-api-package-boundary-review plus python-typing-contract-review |
...
| `security-privacy-supply-chain` | python-security-supply-chain-review; add python-native-extension-review when applicable |
...
- pyproject/setup/build metadata, lock and environment-manager state
- requires-python and supported interpreter/platform matrix
- package/import/public typing surfaces and py.typed/stubs
- runtime validators, serializers, migrations and persistence
- async tasks, queues, processes, context managers and native extensions
- tests, fixtures, wheels/sdists and repository commands
...
- repository-authoritative format/lint/type/test/build commands
- wheel/sdist construction and clean-environment consumer install
- runtime validation, async/process/resource and migration evidence
- native-extension ABI evidence when applicable
- python-evidence-reproducer as the principal EvidenceReceipt v2 adapter
...
```text
immutable bbk-profile-python-0.1.0-alpha.3.zip
SHA-256 companion
package manifest
release notes
migration guide
