#!/usr/bin/env python3
"""Build a deterministic bbk-profile-python zip and companion artifacts."""
...
TOP = f"bbk-profile-python-{VERSION}"
...
def build_manifest() -> dict[str, Any]:
    ...
    risk = json.loads((ROOT / "mappings" / "risk-trigger-map.json").read_text(encoding="utf-8"))
    gates = json.loads((ROOT / "gates" / "python-gates.json").read_text(encoding="utf-8"))
    fixture_files = [item for item in files if item["path"].startswith("fixtures/")]
    structure_fixtures = [item for item in fixture_files if item["path"].startswith("fixtures/alpha4/")]
    return {
        ...
        "trigger_count": len(risk.get("triggers", [])),
        "gate_recipe_count": len(gates.get("recipes", [])),
        "fixture_file_count": len(fixture_files),
        "alpha4_structure_fixture_file_count": len(structure_fixtures),
        ...
        "source_provenance_sha256": sha256_file(ROOT / "sources" / "original-digests.json"),
        "authority_disclaimer": "This optional BBK profile adds procedure and gate recipes; it grants no tools, environments, effects, completion, readiness, or release authority.",
    }
...
def qualification_checks() -> None:
    # Parse every Python source in memory.  Qualification must not create
    # __pycache__ or .pyc files inside fixtures or the package source tree.
    for path in [item for item in package_files() if item.suffix == ".py"]:
        source = path.read_bytes()
        compile(source, str(path.relative_to(ROOT)), "exec", dont_inherit=True)
    for path in [ROOT / "PROFILE.json", *sorted((ROOT / "schemas").glob("*.json")), *sorted((ROOT / "mappings").glob("*.json")), *sorted((ROOT / "gates").glob("*.json")), *sorted((ROOT / "sources").glob("*.json"))]:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not args.skip_tests:
        ...
        if after["root_sha256"] != manifest["root_sha256"] or after["files"] != manifest["files"]:
            raise RuntimeError("qualification mutated the manifested package source tree")
