...
`bbk-profile-python 0.1.0-alpha.3` is package-qualified against the exact supplied BBK `0.1.0-alpha.8` core and the `bbk.profile-capability.v1` protocol.
...
- 47 tests executed: 46 passed; 1 optional historical BBK alpha.3 comparison-core test skipped because that legacy core reference was not supplied; the supplied alpha.8 core was qualified separately (10 alpha.8 dispatch tests plus 37 inherited/successor tests);
- all 15 skill files verified byte-identical to alpha.2;
